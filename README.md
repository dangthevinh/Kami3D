...
# --- Auto-pilot (Phase 21) — optional; the switch itself lives in /admin/models
...
| `CLERK_SUPABASE_JWT_TEMPLATE` | server | Which Clerk token Supabase should be given. `session` means the customized **session token** — the current path, set up by Clerk's *Connect with Supabase* integration — and any other value is a Clerk JWT template name. Leave it unset until Clerk is connected to Supabase: without it, personal rows are written with the service role and the application filters by `user_id`; with it, the database's row level security enforces ownership instead. `npm run verify:clerk-rls` is the check |
